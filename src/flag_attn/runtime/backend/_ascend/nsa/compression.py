...
from flag_attn.runtime.backend._ascend.ops.bwd_preprocess import parallel_attn_bwd_preprocess
from flag_attn.FLA.index import (
    prepare_chunk_indices,
    prepare_chunk_offsets,
    prepare_token_indices,
)
from flag_attn.FLA.utils import autotune_cache_kwargs, exp, log
...
def parallel_nsa_compression_bwd_kernel_dq(
    ...
    # With no complete compressed block the output is constant zero. Make
    # this explicit at the store: 910B can leave a zero-trip accumulator
    # undefined even though its initializer is tl.zeros.
    b_dq = tl.where(NC > 0, b_dq * scale, 0.0)
...
def parallel_nsa_compression_fwd(
    ...
    if split:
        max_core_t = max(1, 65535 // (NV * B * H))
        # Match the main NSA path: large-T/high-BH launches need a smaller
        # chunk on 910B to stay below the AICore watchdog threshold.
        default_chunk_t = 256 if (T >= 65536 or B * H >= 64) else 1024
        chunk_t = min(
            max_core_t,
            int(os.environ.get("FLAGGEMS_NSA_FWD_CHUNK_T", str(default_chunk_t))),
        )
        for start in range(0, T, chunk_t):
            end = min(start + chunk_t, T)
            parallel_nsa_compression_fwd_kernel[(end - start, NV, B * H)](
                q=q,
                k=k,
                v=v,
                o=o,
        ...
        parallel_nsa_compression_fwd_kernel[(T, NV, B * H)](
            q=q,
            k=k,
            v=v,
            o=o,
...
def parallel_nsa_compression(
    ...
    Args:
        ...
        k: Compressed keys of shape ``[B, ceil(T / block_size), H, K]``.
        v: Compressed values of shape ``[B, ceil(T / block_size), H, V]``.
        block_size: Compression block size (default 64).
        scale: Attention scale. If ``None``, defaults to ``K ** -0.5``.
        cu_seqlens: Optional cumulative sequence lengths for
    ...
    if q.device.type != "npu":
        raise ValueError("Ascend NSA compression requires NPU tensors")
    if scale is None:
        scale = k.shape[-1] ** -0.5
