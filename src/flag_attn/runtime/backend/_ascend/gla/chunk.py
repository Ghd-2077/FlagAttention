...
from flag_attn.runtime.backend._ascend.ops.cumsum import chunk_local_cumsum
...
def chunk_gla_fwd_A_kernel_intra_sub_inter(
    ...
    i_t, i_c = tl.program_id(0), tl.program_id(1)
...
def chunk_gla_fwd_A_kernel_intra_sub_intra(
    ...
    i_t, i_i = tl.program_id(0), tl.program_id(1)
    i_bh = tl.program_id(2) + BH_OFFSET
    i_b, i_h = i_bh // H, i_bh % H
    i_j = i_i
    if IS_VARLEN:
...
def chunk_gla_fwd_A_kernel_intra_sub_intra_split(
    ...
    i_k, i_tc = tl.program_id(0), tl.program_id(1)
    i_bh = tl.program_id(2) + BH_OFFSET
    i_b, i_h = i_bh // H, i_bh % H
...
def chunk_gla_fwd_kernel_o(
    ...
    i_v, i_t = tl.program_id(0), tl.program_id(1)
    i_bh = tl.program_id(2) + BH_OFFSET
...
if HAS_TLE:
    ...
    @triton.autotune(
        configs=[
            ...
            for num_warps in [2, 4, 8]
            for num_stages in [2, 3, 4]
            # Ascend 910B TLE: this boundary-tile combination is incorrect.
            if not (BV == 128 and B_VCHUNK == 4)
        ],
        key=["BT", "HV", "STATE_V_FIRST", "V"],
    )
    @triton.jit(do_not_specialize=["T"])
    def chunk_gla_fwd_kernel_o_tle(
        ...
        i_vg, i_t = tl.program_id(0), tl.program_id(1)
        i_bh = tl.program_id(2) + BH_OFFSET
        i_v_base = i_vg * B_VCHUNK
        # Avoid entirely out-of-range V tiles in statically unrolled TLE loads.
        V_TILES: tl.constexpr = (V + BV - 1) // BV
        ACTIVE_VCHUNK: tl.constexpr = B_VCHUNK if B_VCHUNK < V_TILES else V_TILES
        i_b, i_hv = i_bh // HV, i_bh % HV
        i_h = i_hv // (HV // H)
        if IS_VARLEN:
            i_tg = i_t.to(tl.int64)
        ...
        if ACTIVE_VCHUNK <= 1:
            ...
            for i_k in range(n_k_tiles):
                ...
                for d in tl.static_range(ACTIVE_VCHUNK):
                    i_v = i_v_base + d
...
def chunk_gla_bwd_kernel_intra(
    ...
    i_kc, i_t = tl.program_id(0), tl.program_id(1)
    i_bh = tl.program_id(2) + BH_OFFSET
    i_b, i_h = i_bh // H, i_bh % H
...
def chunk_gla_bwd_kernel_inter(
    ...
    i_k, i_t = tl.program_id(0), tl.program_id(1)
    i_bh = tl.program_id(2) + BH_OFFSET
    i_b, i_h = i_bh // H, i_bh % H
    if IS_VARLEN:
        i_tg = i_t
...
def chunk_gla_fwd_intra_gk(
    ...
    grid_xy = NT * NC * NC
    for bh_offset, bh_count in _split_bh_ranges(grid_xy, B * H, q.device):
        grid = (NT, NC * NC, bh_count)
...
def chunk_gla_fwd_o_gk(
    ...
    def grid(meta, bh_count):
        return (triton.cdiv(V, meta["BV"]), NT, bh_count)

    def grid_tle(meta, bh_count):
        return (triton.cdiv(V, meta["BV"] * meta.get("B_VCHUNK", 1)), NT, bh_count)

    _use_tle = HAS_TLE and os.environ.get("FLAG_ATTN_ASCEND_GLA_TLE", "1") != "0"
    if _use_tle:
    ...
    # The smallest V tile (64) gives a conservative upper bound for all
    # forward autotune candidates, including TLE's B_VCHUNK variants.
    grid_xy = triton.cdiv(V, 64) * NT
    for bh_offset, bh_count in _split_bh_ranges(grid_xy, B * HV, q.device):
        kernel[lambda meta, bh_count=bh_count: grid_fn(meta, bh_count)](
            q=q,
            v=v,
            g=g,
            h=h,
...
def chunk_gla_bwd_dv(
    ...
    grid_xy = triton.cdiv(V, min(BV_LIST)) * NT
    for bh_offset, bh_count in _split_bh_ranges(grid_xy, B * H, k.device):
        chunk_gla_bwd_kernel_dv[lambda meta, bh_count=bh_count: grid(meta, bh_count)](
            k=k,
            g=g,
            A=A,
            do=do,
...
def chunk_gla_bwd_dqkg(
    ...
    def grid(meta, bh_count):
        return (triton.cdiv(K, meta["BK"]), NT, bh_count)

    grid_xy = triton.cdiv(K, min(BK_LIST)) * NT
    for bh_offset, bh_count in _split_bh_ranges(grid_xy, B * H, q.device):
        chunk_gla_bwd_kernel_inter[lambda meta, bh_count=bh_count: grid(meta, bh_count)](
            q=q,
            k=k,
            v=v,
            g=g,
...
def chunk_gla(
    ...
    Examples::
        ...
        >>> from flag_attn.runtime.backend._ascend.gla import chunk_gla
        # inputs with equal lengths
        >>> B, T, H, K, V = 4, 256, 4, 64, 64
        >>> q = torch.randn(B, T, H, K, device='npu', dtype=torch.bfloat16)
        >>> k = torch.randn_like(q)
        >>> v = torch.randn_like(q)
        >>> g = F.logsigmoid(torch.randn_like(q))
        >>> h0 = torch.randn(B, H, K, V, device='npu', dtype=torch.float32)
        >>> o, ht = chunk_gla(
            q, k, v, g,
            initial_state=h0,
        ...
        >>> cu_seqlens = q.new_tensor([0, 256, 512, 768, 1024], dtype=torch.long)
        >>> o, ht = chunk_gla(
            q, k, v, g,
            initial_state=h0,
            output_final_state=True,
    ...
    if q.device.type != "npu":
        raise ValueError("Ascend GLA requires NPU tensors")
    if cu_seqlens is not None:
        if q.shape[0] != 1:
            raise ValueError(
